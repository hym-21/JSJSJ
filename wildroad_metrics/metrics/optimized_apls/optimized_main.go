...
func GraphDensify(g *graph) *graph {
	// Compute densified segments in parallel per edge, then build sequentially
	// to preserve deterministic node/edge ordering and logic-equivalent results.
	type seg struct {
		loc1 [2]float64
		loc2 [2]float64
	}

	segsByEdge := make([][]seg, len(g.Edges))
				...
				if d > 3.0 {
					...
					for i := 0; i < n; i++ {
						...
						if i == 0 {
							...
							local = append(local, seg{loc1: loc1, loc2: loc2})
						} else if i == n-1 {
					...
					segsByEdge[eidx] = local
				} else {
					loc1 := g.Nodes[n1]
					loc2 := g.Nodes[n2]
					segsByEdge[eidx] = []seg{{loc1: loc1, loc2: loc2}}
				}
			}
		}()
	}

	for eidx := range g.Edges {
		jobs <- eidx
	}
	close(jobs)
	wg.Wait()

	// Build densified graph sequentially in the original edge order.
	ng := new(graph)
	ng.loc2index = make(map[string]int)
	ng.neighbors = make(map[int]map[int]bool)

	for eidx := 0; eidx < len(g.Edges); eidx++ {
		for _, s := range segsByEdge[eidx] {
			ng.addEdge(s.loc1, s.loc2)
		}
	}
...
func apls_one_way(graph_gt *graph, graph_prop *graph, ret chan float64) {
	...
	// Parallelize computing prop graph shortest paths
	...
	jobsSP := make(chan int, workerCount*2)
	resultsSP := make(chan spResult, len(control_point_prop_list))
	var wgSP sync.WaitGroup

	wgSP.Add(workerCount)
	for w := 0; w < workerCount; w++ {
		go func() {
			defer wgSP.Done()
			for src := range jobsSP {
				dist, _ := graph_prop.ShortestPaths(src, control_point_prop_list)
				resultsSP <- spResult{src: src, dist: dist}
			}
		}()
	}
	...
	for r := range resultsSP {
		shortest_paths_prop[r.src] = r.dist
		counter += 1
		if counter % 100 == 0 {
	...
	// Parallelize computing gt graph shortest paths
	jobsSP2 := make(chan int, workerCount*2)
	resultsSP2 := make(chan spResult, len(control_point_gt_list))
	var wgSP2 sync.WaitGroup

	wgSP2.Add(workerCount)
	for w := 0; w < workerCount; w++ {
		go func() {
			defer wgSP2.Done()
			for src := range jobsSP2 {
				dist, _ := graph_gt.ShortestPaths(src, control_point_gt_list)
				resultsSP2 <- spResult{src: src, dist: dist}
			}
		}()
	}
	...
	for r := range resultsSP2 {
		shortest_paths_gt[r.src] = r.dist
		counter += 1
		if counter % 100 == 0 {
	...
	// Parallelize pairwise aggregation across cp1
	type agg struct { sum float64; cc float64; pairs int }
	cp1Jobs := make(chan int, workerCount*2)
	resultsAgg := make(chan agg, len(control_point_gt))
	var wgAgg sync.WaitGroup

	var cp1List []int
	for cp1 := range control_point_gt { cp1List = append(cp1List, cp1) }

	wgAgg.Add(workerCount)
	for w := 0; w < workerCount; w++ {
		go func() {
			defer wgAgg.Done()
			for cp1_gt := range cp1Jobs {
				cp1_prop := control_point_gt[cp1_gt]
				localSum := 0.0
				localCC := 0.0
				localPairs := 0
				for cp2_gt, cp2_prop := range control_point_gt {
					if cp2_gt <= cp1_gt { continue }
					localPairs += 1
				...
				resultsAgg <- agg{sum: localSum, cc: localCC, pairs: localPairs}
			}
		}()
	}

	go func() {
		for _, cp1 := range cp1List { cp1Jobs <- cp1 }
		close(cp1Jobs)
		wgAgg.Wait()
		close(resultsAgg)
	}()

	for r := range resultsAgg {
		sum += r.sum
		cc += r.cc
		pair_num += r.pairs
	}
